...
resource "aws_ecs_task_definition" "dev_ecs_service" {
  ...
  container_definitions    = "${file("${path.module}/files/dev-taskdefinition.json")}"
  ...
  depends_on = [
    local_file.task_definition_dev
  ]
}
...
#   network_mode             = "bridge"
#   task_role_arn            = aws_iam_role.ecs_task_role_dev.arn
#   container_definitions    = jsonencode(templatefile("${path.module}/templates/taskdefinition.tpl", {
